...
func (setup *FabricSetup) InstallAndInstantiateCC() error {
	...
	if err != nil {
		return errors.WithMessage(err, "failed to create haochain package")
	}
	...
	if err != nil {
		return errors.WithMessage(err, "failed to install haochain")
	}
	fmt.Println("Chaincode installed")

	// Set up chaincode policy
	ccPolicy := cauthdsl.SignedByAnyMember([]string{"org1.hf.haochain.io"})
	...
	if err != nil || resp.TransactionID == "" {
		return errors.WithMessage(err, "failed to instantiate the haochain")
	}
